...
export function useAddProductForm() {
  ...
  const [formData, setFormData] = useState(INITIAL_PRODUCT_FORM_DATA);
  ...
  useEffect(() => {
    if (existingProduct) {
      ...
      const usCountryEntry = Array.isArray(p.countries)
        ? p.countries.find(
            (c) => c.currency === "USD" || c.country === "United States" || c.country?.code === "US"
          )
        : null;
      const caCountryEntry = Array.isArray(p.countries)
        ? p.countries.find(
            (c) => c.currency === "CAD" || c.country === "Canada" || c.country?.code === "CA"
          )
        : null;
        ...
        Array.isArray(p.variants) && p.variants.length > 0
          ? p.variants.map((v, i) => {
              const vUs = Array.isArray(v.countries)
                ? v.countries.find(
                    (c) =>
                      c.currency === "USD" || c.country === "United States" || c.country?.code === "US"
                  )
                : null;
              ...
              const totalVStock =
                v.stock !== undefined ? v.stock : v.stock_quantity !== undefined ? v.stock_quantity : 50;
              const vStockUsd =
                vUs?.stock !== null && vUs?.stock !== undefined
                  ? String(vUs.stock)
                  : String(Math.round(totalVStock * 0.6));
              const vStockCad =
                vCa?.stock !== null && vCa?.stock !== undefined
                  ? String(vCa.stock)
                  : String(Math.max(0, totalVStock - Math.round(totalVStock * 0.6)));
              ...
              return {
                ...
                price_usd:
                  vUs?.price !== null && vUs?.price !== undefined
                    ? String(vUs.price)
                    : v.price_usd !== null && v.price_usd !== undefined
                    ? String(v.price_usd)
                    : "",
                old_price_usd:
                  vUs?.oldPrice !== null && vUs?.oldPrice !== undefined
                    ? String(vUs.oldPrice)
                    : v.old_price_usd !== null && v.old_price_usd !== undefined
                    ? String(v.old_price_usd)
                    : "",
                stock_usd: vStockUsd,
                price_cad:
                  vCa?.price !== null && vCa?.price !== undefined
                    ? String(vCa.price)
                    : v.price_cad !== null && v.price_cad !== undefined
                    ? String(v.price_cad)
                    : "",
                old_price_cad:
                  vCa?.oldPrice !== null && vCa?.oldPrice !== undefined
                    ? String(vCa.oldPrice)
                    : v.old_price_cad !== null && v.old_price_cad !== undefined
                    ? String(v.old_price_cad)
                    : "",
                stock_cad: vStockCad,
      ...
      const resolvedPriceUsd =
        usCountryEntry?.price !== null && usCountryEntry?.price !== undefined
      ...
      const resolvedOldPriceUsd =
        usCountryEntry?.oldPrice !== null && usCountryEntry?.oldPrice !== undefined
          ? String(usCountryEntry.oldPrice)
          : p.old_price_usd
          ? String(p.old_price_usd)
          : p.oldPrice
          ? String(p.oldPrice)
          : "";

      const resolvedPriceCad =
        caCountryEntry?.price !== null && caCountryEntry?.price !== undefined
      ...
      const resolvedOldPriceCad =
        caCountryEntry?.oldPrice !== null && caCountryEntry?.oldPrice !== undefined
          ? String(caCountryEntry.oldPrice)
          : p.old_price_cad
          ? String(p.old_price_cad)
          : "";

      const resolvedStock =
        p.stock !== undefined ? p.stock : p.stock_quantity !== undefined ? p.stock_quantity : 100;
      const resolvedStockUsd =
        usCountryEntry?.stock !== null && usCountryEntry?.stock !== undefined
          ? String(usCountryEntry.stock)
          : String(Math.round(resolvedStock * 0.6));
      const resolvedStockCad =
        caCountryEntry?.stock !== null && caCountryEntry?.stock !== undefined
          ? String(caCountryEntry.stock)
          : String(Math.max(0, resolvedStock - Math.round(resolvedStock * 0.6)));

      let loadedHighlights = [];
      if (Array.isArray(p.keyHighlights) && p.keyHighlights.length > 0) {
        loadedHighlights = p.keyHighlights.map((kh) =>
          typeof kh === "string"
            ? { label: "Feature", value: kh }
            : { label: kh.label || "Highlight", value: kh.value || "" }
        );
      } else if (Array.isArray(p.key_highlights) && p.key_highlights.length > 0) {
        loadedHighlights = p.key_highlights.map((kh) =>
          typeof kh === "string"
            ? { label: "Feature", value: kh }
            : { label: kh.label || "Highlight", value: kh.value || "" }
        );
      ...
      setFormData({
        ...
        product_type:
          p.product_type || p.type?.toLowerCase() || (pVariants.length > 0 ? "variable" : "simple"),
        ...
        variants: pVariants.length > 0 ? pVariants : INITIAL_PRODUCT_FORM_DATA.variants,
      });
    }
  }, [existingProduct]);
  ...
  const addHighlightRow = () => {
    setFormData((prev) => ({
      ...prev,
      key_highlights: [...(prev.key_highlights || []), { label: "Highlight", value: "" }],
    }));
  };
  ...
  const handleSubmit = (e) => {
    ...
    if (!isVariable) {
      ...
      if (firstMissingPrice) {
        toast.error(
          ...
          `Please enter the USD price for variant "${firstMissingPrice.variant_name}".`
        );
        return;
      }
    }
    ...
    const usCountry = countries.find(
      (c) => c.code === "US" || c.name?.toLowerCase().includes("united states")
    );
    const caCountry = countries.find(
      (c) => c.code === "CA" || c.name?.toLowerCase().includes("canada")
    );
    ...
    const processedVariants = isVariable
      ? formData.variants.map((v, i) => {
          ...
          const vPriceCad = v.price_cad
            ? parseFloat(v.price_cad)
            : parseFloat((vPriceUsd * 1.35).toFixed(2));
          const vOldPriceCad = v.old_price_cad
            ? parseFloat(v.old_price_cad)
            : vOldPriceUsd
            ? parseFloat((vOldPriceUsd * 1.35).toFixed(2))
            : null;
    ...
    const baseOldUsdPrice = isVariable
      ? formData.variants[0]?.old_price_usd
        ? parseFloat(formData.variants[0]?.old_price_usd)
        : null
      : formData.old_price_usd
      ? parseFloat(formData.old_price_usd)
      : null;

    const baseCadPrice = isVariable
      ? formData.variants[0]?.price_cad
        ? parseFloat(formData.variants[0]?.price_cad)
        : parseFloat((baseUsdPrice * 1.35).toFixed(2))
      : formData.price_cad
      ? parseFloat(formData.price_cad)
      : parseFloat((baseUsdPrice * 1.35).toFixed(2));

    const baseOldCadPrice = isVariable
      ? formData.variants[0]?.old_price_cad
        ? parseFloat(formData.variants[0]?.old_price_cad)
        : baseOldUsdPrice
        ? parseFloat((baseOldUsdPrice * 1.35).toFixed(2))
        : null
      : formData.old_price_cad
      ? parseFloat(formData.old_price_cad)
      : baseOldUsdPrice
      ? parseFloat((baseOldUsdPrice * 1.35).toFixed(2))
      : null;
    ...
    if (usCountry) {
      productCountries.push({
        ...
        stock: isVariable
          ? processedVariants.reduce((sum, v) => {
  ...
  return {
    ...
    handleSubmit,
    navigateToList: () => navigate(ROUTES.ADMIN.PRODUCTS),
  };
}

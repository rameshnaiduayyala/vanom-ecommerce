...
export function DeleteCompanyModal({ company, isOpen, onClose, onConfirm, isPending }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-xs animate-in fade-in duration-200">
      <div className="bg-white rounded-3xl border border-border shadow-2xl max-w-sm w-full p-6 text-center space-y-4">
        ...
        <div>
          <h3 className="font-bold text-base text-[#0F2B1C]">Delete Corporate Entity?</h3>
          <p className="text-xs text-text-muted mt-1 leading-relaxed">
            Are you sure you want to permanently delete{" "}
            <strong className="text-text-primary">
              {company.tradingName || company.legalName}
            </strong>
            ? This will remove all associated members and wholesale profiles.
          </p>
        </div>

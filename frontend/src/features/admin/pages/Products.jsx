                  ...
                  {filteredProducts.length === 0 ? (
                    ...
                    filteredProducts.map((p) => {
                      const usCountry = Array.isArray(p.countries)
                        ? p.countries.find((c) => c.currency === "USD" || c.country?.code === "US" || c.country === "United States")
                        : null;
                      const caCountry = Array.isArray(p.countries)
                        ? p.countries.find((c) => c.currency === "CAD" || c.country?.code === "CA" || c.country === "Canada")
                        : null;

                      const priceUsd = usCountry?.price ?? p.basePrice ?? p.price_usd ?? p.priceUS ?? p.pricing?.US?.retailPrice ?? 0;
                      const priceCad = caCountry?.price ?? p.price_cad ?? p.priceCA ?? p.pricing?.CA?.retailPrice ?? (priceUsd ? (Number(priceUsd) * 1.35).toFixed(2) : 0);
                      const stockUs = usCountry?.stock ?? Math.round((p.stock ?? 100) * 0.6);
                      const stockCa = caCountry?.stock ?? Math.max(0, (p.stock ?? 100) - Math.round((p.stock ?? 100) * 0.6));
                      const totalStock = p.stock ?? p.stock_quantity ?? 100;
                      const isVariable = p.type === "VARIABLE" || (Array.isArray(p.variants) && p.variants.length > 0);

                      const primaryImg = (Array.isArray(p.images) && p.images.length > 0)
                        ? (typeof p.images[0] === "string" ? p.images[0] : (p.images[0]?.url || p.images[0]?.file?.url))
                        : (p.image || "https://images.unsplash.com/photo-1586201375761-83865001e31c?auto=format&fit=crop&w=400&q=80");
                        ...
                        <tr key={p.id} className="hover:bg-surface-muted/50 transition-colors">
                          <td className="p-4">
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                ...
                                <span className="text-[10px] text-text-muted">{p.brand?.name || p.brand || "Vanom Organics"}</span>
                              </div>
                            </div>
                          </td>
                          <td className="p-4 font-mono text-text-secondary font-medium">{p.sku || "N/A"}</td>
                          <td className="p-4">
                            <Badge variant="default" size="sm">
                              {typeof p.category === "object" ? p.category?.name : (p.category || (p.categories?.[0]?.category?.name) || "General")}
                            </Badge>
                          </td>
                          <td className="p-4">
                            <div className="flex flex-wrap gap-1 items-center">
                              <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${isVariable ? "bg-purple-100 text-purple-800" : "bg-slate-100 text-slate-700"}`}>
                          ...
                          <td className="p-4">
                            <div className="space-y-0.5">
                              <span className="font-bold text-slate-900">${Number(priceUsd).toFixed(2)}</span>
                              <div className="text-[10px] text-emerald-700 font-semibold">{stockUs} in stock</div>
                            </div>
                          </td>
                          <td className="p-4">
                            <div className="space-y-0.5">
                              <span className="font-bold text-blue-900">CA${Number(priceCad).toFixed(2)}</span>
                              <div className="text-[10px] text-blue-700 font-semibold">{stockCa} in stock</div>
                            </div>
                          </td>
                          <td className="p-4">
                            <span className={`font-bold ${totalStock < 20 ? "text-amber-600" : "text-emerald-700"}`}>
                              {totalStock} units
                            </span>
                          </td>

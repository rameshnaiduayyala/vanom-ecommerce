        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-24 relative z-10 flex flex-col md:flex-row items-center justify-between gap-10">
          <div className="max-w-2xl space-y-5">
            <Badge variant="gold" size="md" className="font-bold tracking-wide uppercase">
              ✨ Enterprise Multi-Country Agro Platform
            </Badge>
            <h1 className="text-3xl sm:text-5xl font-extrabold tracking-tight leading-tight">
              Direct-from-Source Organic Soils & Horticulture Supplies
            </h1>
            <p className="text-sm sm:text-base text-slate-200 leading-relaxed max-w-xl">
              Engineered growing media, commercial planters, and natural fertilizers delivered to your doorstep in India, the US, and the UK.
            </p>
          ...
          <div className="w-full md:w-80 bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20 text-white space-y-4">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold uppercase tracking-wider text-gold-300">B2B Wholesale</span>
              <span className="text-xs bg-gold-400 text-slate-900 font-bold px-2 py-0.5 rounded">Up to 30% Off</span>
            </div>
            <h4 className="text-base font-bold">Buying by the Pallet or Truckload?</h4>
            <p className="text-xs text-slate-200 leading-relaxed">
              Unlock Tier 3 wholesale pricing, 30-day invoice terms, and dedicated freight shipping.
            </p>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-6 bg-white rounded-xl border border-border shadow-xs">
          <div className="flex items-start gap-3">
            <Leaf className="w-5 h-5 text-brand-600 shrink-0 mt-0.5" />
            <div>
              <h4 className="text-xs font-bold text-text-primary">100% Bio-Active</h4>
              <p className="text-[11px] text-text-muted">Enriched natural formulas</p>
            </div>
          </div>
          <div className="flex items-start gap-3">
            <Truck className="w-5 h-5 text-brand-600 shrink-0 mt-0.5" />
            <div>
              <h4 className="text-xs font-bold text-text-primary">Express & Freight</h4>
              <p className="text-[11px] text-text-muted">Local & cross-border delivery</p>
            </div>
          </div>
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-text-muted">Automated volume discounts</p>
            </div>
          </div>
          <div className="flex items-start gap-3">
            <ShieldCheck className="w-5 h-5 text-brand-600 shrink-0 mt-0.5" />
            <div>
              <h4 className="text-xs font-bold text-text-primary">Authoritative Tax</h4>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-xs text-text-muted">Explore tailored inputs for cultivation and landscaping</p>
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4">
          {categories.map((cat) => (
            ...
            >
              ...
              <span className="text-[11px] text-text-muted mt-1">{cat.count} Items</span>
            </Link>
          ))}
        </div>
      </section>
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-xl font-bold text-text-primary">Bestselling Inputs & Foliage</h2>
              <Badge variant="brand" size="sm">
                Market: {country.name} ({country.currency})
              </Badge>
            </div>
            <p className="text-xs text-text-muted">High-demand products with immediate stock availability</p>
          </div>
          <Link to={ROUTES.PRODUCTS} className="text-xs font-semibold text-brand-600 hover:text-brand-700 flex items-center gap-1">

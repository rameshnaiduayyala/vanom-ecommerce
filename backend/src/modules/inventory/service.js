...
/**
 * InventoryService
 * Direct Prisma stock management, atomic reservations, releases, and adjustments
 */
export class InventoryService {
  async getStock(variantId, warehouseId = null) {
  ...
  async reserveStock({ variantId, quantity, orderId = null, warehouseId = null }, tx = null) {
    const db = tx || prisma;
    const where = { variantId };
    if (warehouseId) where.warehouseId = warehouseId;
    ...
    const targetItem = inventoryItems.find((item) => item.onHand - item.reserved >= quantity);
  ...
  async releaseReservation(reservationId, tx = null) {
    const db = tx || prisma;
  ...
  async consumeReservation(reservationId, tx = null) {
    const db = tx || prisma;
    const reservation = await db.inventoryReservation.findUnique({
      where: { id: reservationId },
    });
  ...
  async adjustStock({ warehouseId, locationId, variantId, quantity, type = "ADJUSTMENT", reason }, tx = null) {
    const db = tx || prisma;
